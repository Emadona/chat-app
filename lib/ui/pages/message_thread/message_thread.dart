...
import 'package:chat/ui/widgets/message_thread/receiver_message.dart';
import 'package:chat/ui/widgets/message_thread/sender_message.dart';
import 'package:chat/ui/widgets/shared/header_status.dart';
import 'package:chat/ui/widgets/shared/weather_status.dart';
...
import '../../../colors.dart';
import 'message_thread_router.dart';
...
class _MessageThreadState extends State<MessageThread> {
  ...
  @override
  void initState() {
    BlocProvider.of<WeatherBloc>(context).add(GetWeather(widget.receiver.city));
                      ...
                      children: [
                            ...
                            children: [
                              DecoratedBox(
                                ...
                                child: Padding(
                                  ...
                                  child: Text(
                                    messages[idx].message.contents.trim(),
                                    softWrap: true,
                                ...
                                child: Align(
                                  ...
                                  child: Row(
                                    children: [
                                      ...
                                      messages[idx].isLike == IsLikeStatus.like
                                          ? IconButton(
                                              icon: Icon(
                        ...
                        CircleAvatar(
                          ...
                          child: ClipRRect(
                            ...
                            child: Image.memory(
                              base64Decode(receiver.photoUrl),
                              width: 30,
                              height: 30,
                              fit: BoxFit.fill,
                            ),
  ...
  void _sentLike(LocalMessage message) {
    if (message.isLike == IsLikeStatus.notLike) {
      final isLike = IsLike(
        recipient: message.message.from,
        messageId: message.id,
        status: IsLikeStatus.like,
        timestamp: DateTime.now().toString(),
      );
      context.read<IsLikeBloc>().add(IsLikeEvent.onMessageSent(isLike));
    } else {
      final isLike = IsLike(
        recipient: message.message.from,
        messageId: message.id,
        status: IsLikeStatus.notLike,
        timestamp: DateTime.now().toString(),
      );
      context.read<IsLikeBloc>().add(IsLikeEvent.onMessageSent(isLike));
    }
  ...
  void _updateOnLikeReceived() {
    final messageThreadCubit = context.read<MessageThreadCubit>();
    context.read<IsLikeBloc>().stream.listen((state) async {

      if (state is IsLikeReceivedSuccess) {

        await messageThreadCubit.viewModel.updateMessageLikes(state.isLike);
        messageThreadCubit.messages(chatId);
        widget.chatsCubit.chats();
      }
      if (state is IsLikeSentSuccess) {
        await messageThreadCubit.viewModel.updateMessageLikes(state.isLike);
        messageThreadCubit.messages(chatId);
        widget.chatsCubit.chats();
      }
  ...
  void _dispatchTyping(Typing event) {
    ...
    widget.typingNotificationBloc
        .add(TypingNotificationEvent.onTypingEvent(typing));
  }
