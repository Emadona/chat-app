...
//     Map record = await _r.table('typing_events').insert(event.toJson(),
...
//     _changefeed =
//         _r.table('typing_events').filter((event) {
//       return event('to').eq(user.id)
//           .and(_r.expr(userIds).contains(event('from')));
//     }).changes({'include_initial' : true}).run(_connection)
//         .asStream().cast<Feed>().listen((event) {
...
//
//   @override
//   Future<bool> update({TypingEvent event}) {
